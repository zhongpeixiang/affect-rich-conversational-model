...
def model_opts(parser):
    ...
    group.add_argument('-affective_attention', type=str, default=None,
                       choices=['norm', 'matrix_norm', 'bigram_norm'],
                       help="""The affective attention type to use:
                       norm or norm of the product with decoder hidden state""")
    group.add_argument('-affective_attn_strength', type=float, default=0.1,
                       help="""The strength of affective norm in the alignment function""")
    ...
    group.add_argument('-affect_obj_coefficient', type=float, default=0,
                       help="""The strength of affective norm of generated response in the loss function""")
    group.add_argument('-affect_softmax_scaler', type=float, default=1,
                       help="""The scaling factor before softmax computation for affect strength in the loss function""")
    group.add_argument('-kl_loss', action='store_true',
                       help="""Use KL divergence loss for word affect norm""")
    group.add_argument('-vad_loss', action='store_true',
                       help="""Use vad value as a coefficient of cross entropy loss""")
    group.add_argument('-adj_only', action='store_true',
                       help="""Consider adjectives only when computing affect loss""")
    group.add_argument('-emotional_words', default="",
                       help="Path for word embeddings with only emotional words")
    group.add_argument('-lambda_AC', type=float, default=0,
                       help="""lambda in Asghar's paper 2017""")
...
def train_opts(parser):
    ...
    group.add_argument('-pre_word_vecs_dec',
                       ...
                       See README for specific formatting instructions.""")
    group.add_argument('-pre_word_vecs_enc_adj', 
                       help="""If a valid path is specified, then this will load
                       pretrained word embeddings on the encoder side for adjectives only.
                       See README for specific formatting instructions.""")
    group.add_argument('-pre_word_vecs_dec_adj',
                       help="""If a valid path is specified, then this will load
                       pretrained word embeddings on the decoder side for adjectives only.
                       See README for specific formatting instructions.""")
    group.add_argument('-affect_embedding',
                       action='store_true',
                       help="Add affect embedding to pre-trained word embeddings")
    group.add_argument('-affect_strength', type=float, default=0.1,
                    help="Strength of affect embeddings from this file")
    group.add_argument('-normalize_VAD',
                       action='store_true',
                       help="If true, normalize VAD embedding before adding into word embedding")
    group.add_argument('-word_freq', default='', type=str,
                       help="""Path to the word frequency file, which is used to compute 
                       weights in affective attention""")
    group.add_argument('-local_weights', action='store_true',
                       help="""If true, use local weights""")
    # Fixed word vectors
    ...
    group.add_argument('-affect_bias', type=float, default=0,
                       help="The softmax bias in validation based on affect norm")
    ...
    group.add_argument('-evaluate_every', type=int, default=10000,
                       help="Evaluate on validation dataset at this interval.")
    group.add_argument('-exp_host', type=str, default="",
                       help="Send logs to this crayon server.")
    group.add_argument('-exp', type=str, default="",
                       help="Name of the experiment for logging.")
    group.add_argument('-loss_log', type=str, default="",
                       help="Path of file to log crossentropy loss and affective loss")
...
def translate_opts(parser):
    ...
    group.add_argument('-display_1',  action='store_true',
                        help='If true, only display top 1 translations')
    ...
    group.add_argument('-antilm_lambda', type=float, default=0,
                       help="""MMI-antilm model lambda coefficient for unconditioned target response probability""")
    group.add_argument('-antilm_eta', type=int, default=0,
                       help="""MMI-antilm model eta, only first eta tokens need to be taken into account""")
    group.add_argument('-antilm_equal_src', action="store_true",
                       help="""If true, pad dummy src with equal length""")
    group.add_argument('-rerank', action="store_true",
                       help="""Rerank n-best predictions based on affect strength of adjectives""")
    group.add_argument('-adj_vocab', type=str, default="",
                    help="""Path to affective embedding for adjectives used in reranking algorithm""")
    group.add_argument('-lambda_ADBS', type=float, default=0.,
                       help="""lambda parameter for affective divese beam search""")
    group.add_argument('-affective_decoding', action="store_true",
                       help="""If true, apply affective decoding to rerank the most likely word by affect strength and similarity""")
    group.add_argument('-k', type=int, default=10,
                       help="""top k words are considered at each decoding step for reranking""")
    group.add_argument('-sort_AS', action="store_true",
                       help="""If true, consider AS when sorting top k words""")
    group.add_argument('-sort_similarity', action="store_true",
                       help="""If true, consider word similarity when sorting top k words""")
    group.add_argument('-penalize_repeats', action="store_true",
                       help="""If true, discourage repeated words during decoding""")
    group.add_argument('-weighted_AS', type=str, default="",
                       help="""The path of unigram frequency file to compute weighted AS""")
    ...
    group.add_argument('-n_best', type=int, default=1,
                       ...
                       decoded sentences""")
    group.add_argument('-save_attn', type=str, default="",
                       help='File to save decoding attention.')
