...
def make_dataset_iter(datasets, fields, opt, is_train=True, repeat=False):
    """
...
def make_loss_compute(model, tgt_vocab, opt, embedding_copy):
    """
    ...
    if opt.copy_attn:
        ...
        compute = onmt.Loss.NMTLossCompute(
            ...
            label_smoothing=opt.label_smoothing, embedding_copy=embedding_copy, 
            affect_obj_coefficient=opt.affect_obj_coefficient,
            affect_softmax_scaler=opt.affect_softmax_scaler,
            kl_loss=opt.kl_loss,
            vad_loss=opt.vad_loss,
            lambda_AC=opt.lambda_AC,
            loss_log=opt.loss_log if "loss_log" in opt else "")
...
def report_evaluation(valid_stats, num_val_batches):
    print('Validation perplexity: %g' % valid_stats.ppl())
    print('Validation accuracy: %g' % valid_stats.accuracy())
    print('Validation distinct-1, distinct-2: {0:.3f}, {1:.3f}'.format(valid_stats.distinct_1(), valid_stats.distinct_2()))
    print('Validation embed_greedy, embed_avg, embed_extrema: {0:.6f}, {1:.6f}, {2:.6f}'.format(valid_stats.embed_greedy/num_val_batches, \
        valid_stats.embed_avg/num_val_batches, valid_stats.embed_extrema/num_val_batches))
    print('Validation affect_dist, affect_strength: {0:.6f}, {1:.6f}'.format(valid_stats.affect_dist/num_val_batches, \
        valid_stats.affect_strength/num_val_batches))
