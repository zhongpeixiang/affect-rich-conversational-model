...
class RNNDecoderBase(nn.Module):
    ...
    def __init__(self, rnn_type, bidirectional_encoder, num_layers,
                 ...
                 affective_attention=None, affective_attn_strength=0.1, word_freq="", local_weights=False):
        ...
        self.attn = onmt.modules.GlobalAttention(
            ...
            attn_type=attn_type,
            affective_attention=affective_attention,
            affective_attn_strength=affective_attn_strength,
    ...
    def forward(self, input, context, state, context_lengths=None, src=None):
        """
        Args:
            ...
            context_lengths (`LongTensor`): the padded source lengths
                `[batch]`.
            src (`LongTensor`): sequences of padded tokens
                                `[src_len x batch x nfeats]`.
        Returns:
        ...
        hidden, outputs, attns, coverage = self._run_forward_pass(
            input, context, state, context_lengths=context_lengths, src=src)
...
class StdRNNDecoder(RNNDecoderBase):
    ...
    """
    def _run_forward_pass(self, input, context, state, context_lengths=None, src=None):
        """
        ...
        Args:
            ...
            src (`LongTensor`): sequences of src padded tokens
                                `[src_len x batch x nfeats]`.
        Returns:
        ...
        # Input sequence original embedding px
        emb_copy = self.embeddings.embedding_copy(src.squeeze(2)) # src_len x batch x embedding_dim
        emb_now = self.embeddings.word_lut(src.squeeze(2))

        # word frequency as weights in affective attention
        word_freq = None
        if self.word_freq:
            if hasattr(self.embeddings, 'word_freq'):
                word_freq = self.embeddings.word_freq # word_freq: (vocab, )
            else:
                with open("data/glove/word_freq_30000.pkl", "rb") as f:
                    word_freq = torch.FloatTensor(pickle.load(f)).cuda()
            word_freq = word_freq[src.squeeze(2).view(-1).data].view_as(src.squeeze(2)).transpose(0, 1) # (batch, src_len)

        # Run the forward pass of the decoder RNN.
        if isinstance(self.rnn, nn.GRU):
            rnn_output, hidden = self.rnn(emb, state.hidden[0])
        else:
            rnn_output, hidden = self.rnn(emb, state.hidden) # rnn_output: decoder hidden state at last layer for each timestep t, hidden: hidden state at last timestep
        # Result Check
        input_len, input_batch, _ = input.size()
        ...
        # Calculate the attention, add affect embedding and bigram features px
        # Use trainable embedding instead of emb_copy px
        attn_outputs, attn_scores = self.attn(
            rnn_output.transpose(0, 1).contiguous(),  # (output_len, batch, d), decoder hidden state at each timestep
            context.transpose(0, 1),                  # (contxt_len, batch, d), encoder hidden state at each timestep
            context_lengths=context_lengths,
            embedding_now=emb_now.transpose(0, 1), # Pass into original input sequence embedding px
            embedding_copy=emb_copy.transpose(0, 1), 
            word_freq=word_freq)
...
class InputFeedRNNDecoder(RNNDecoderBase):
    ...
    def _run_forward_pass(self, input, context, state, context_lengths=None, src=None):
        ...
        for i, emb_t in enumerate(emb.split(1)):
            ...
            # emb_t: (batch, embedding_dim + hidden_size) px
            rnn_output, hidden = self.rnn(emb_t, hidden)
            attn_output, attn = self.attn(
                rnn_output,
                context.transpose(0, 1),
                context_lengths=context_lengths,
                embedding_copy=emb_copy.transpose(0, 1)) # Pass into original input sequence embedding px
            if self.context_gate is not None:
...
class NMTModel(nn.Module):
    ...
    def forward(self, src, tgt, lengths, dec_state=None):
        ...
        enc_hidden, context = self.encoder(src, lengths) # enc_hidden: last encoder state, context: encoder hidden state at every position
        enc_state = self.decoder.init_decoder_state(src, context, enc_hidden)
        out, dec_state, attns = self.decoder(tgt, context,
                                             enc_state if dec_state is None
                                             else dec_state,
                                             context_lengths=lengths, src=src) # Add src to decoder for affective attention and bigram attention
        if self.multigpu:
